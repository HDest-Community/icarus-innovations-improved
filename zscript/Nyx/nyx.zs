class NyxHandler : EventHandler
{
	override void CheckReplacement(ReplaceEvent e)
	{
		...
		switch (e.Replacement.GetClassName())
		{
			case 'ShotgunReplaces':
				if (random[nyxrand]() <= AceCore.GetScaledChance(4, 12, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "NyxRandom";
				}
				break;
			case 'HDPistol':
				if (random[nyxrand]() <= AceCore.GetScaledChance(24, 96, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "NyxRandom";
				}
				break;
			case 'ShellBoxRandom':
				if (random[nyxrand]() <= AceCore.GetScaledChance(2, 10, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "HDNyxMag";
				}
				break;
			case 'ClipMagPickup':
				if (random[nyxrand]() <= AceCore.GetScaledChance(6, 24, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "HDNyxMag";
				}
				break;
	...
	{
		let NyxAmmo = HDAmmo(e.Thing);
		if (!NyxAmmo)
		{
			return;
		}
		if (NyxAmmo.GetClassName() == 'HDRevolverAmmo')
		{
			NyxAmmo.ItemsThatUseThis.Push("HDNyx");
		}
	}
}
