class BitchHandler : EventHandler
{
	override void CheckReplacement(ReplaceEvent e)
	{
		...
		switch (e.Replacement.GetClassName())
		{
			case 'ChaingunReplaces':
				if (random[bitchrand]() <= 64)
				{
					e.Replacement = "BitchRandom";
				}
				break;
		}
	}
	...
	{
		let BitchAmmo = HDAmmo(e.Thing);
		if (!BitchAmmo)
		{
			return;
		}

		switch (BitchAmmo.GetClassName())
		{
			case 'FourMilAmmo':
			case 'HDRocketAmmo':
				BitchAmmo.ItemsThatUseThis.Push("HDBitch");
				break;
		}
	}
}
