class UMPHandler : EventHandler
{
	override void CheckReplacement(ReplaceEvent e)
	{
		...
		{
			case 'ClipBoxPickup':
				if (random[umprand]() <= AceCore.GetScaledChance(16, 64, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "UMPrandom";
				}
				break;
			case 'ClipMagPickup':
				if (random[bljackrand]() <= AceCore.GetScaledChance(16, 64, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "HDUMPMag";
				}
				break;
		}
	}
	...
	{
		let UMPAmmo = HDAmmo(e.Thing);
		if (!UMPAmmo)
		{
			return;
		}
		if (UMPAmmo.GetClassName() == 'HD45ACPAmmo')
		{
			UMPAmmo.ItemsThatUseThis.Push("HDUMP");
		}
	}
}
