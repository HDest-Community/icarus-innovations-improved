...
class FrontierHandler : EventHandler
{
	...
	{
		let FrontierAmmo = HDAmmo(e.Thing);
		if (!FrontierAmmo)
		{
			return;
		}
		if (FrontierAmmo.GetClassName() == 'SevenMilAmmo')
		{
			FrontierAmmo.ItemsThatUseThis.Push("HDFrontier");
		}
		if (FrontierAmmo.GetClassName() == 'HD7mClip')
		{
			FrontierAmmo.ItemsThatUseThis.Push("HDFrontier");
		}
	}
