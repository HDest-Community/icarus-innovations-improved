class USPHandler : EventHandler
{
	override void CheckReplacement(ReplaceEvent e)
	{
		...
		{
			case 'HDPistol':
				if (random[usprand]() <= AceCore.GetScaledChance(24, 96, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "USPRandom";
				}
				break;
			case 'ClipMagPickup':
				if (random[usprand]() <= AceCore.GetScaledChance(12, 48, acl_spawnscale_min, acl_spawnscale_max))
				{
					e.Replacement = "HDUSPMag";
				}
				break;
		}
	}
	...
	{
		let USPAmmo = HDAmmo(e.Thing);
		if (!USPAmmo)
		{
			return;
		}
		if (USPAmmo.GetClassName() == 'HD45ACPAmmo')
		{
			USPAmmo.ItemsThatUseThis.Push("HDUSP");
		}
	}
}
